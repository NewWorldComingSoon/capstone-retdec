...
static void update_pub_insn(cs_insn *pub, InternalInstruction *inter)
{
	...
	pub->detail->x86.encoding.imm_offset[0] = inter->immediateOffset[0];
	pub->detail->x86.encoding.imm_offset[1] = inter->immediateOffset[1];
		...
		*size = (uint16_t)insn.length;
		...
		if (result) {
			...
			if (insn.consumedDisplacement) {
				instr->displace_offset =
				    insn.displacementOffset;
				instr->displacement_raw_size =
				    insn.displacement_raw_size;
			}
			while (insn.numImmediatesConsumed--) {
				instr->imm_offset[insn.numImmediatesConsumed] =
				    insn.immediateOffset
					[insn.numImmediatesConsumed];
				instr->imm_read_size
				    [insn.numImmediatesConsumed] =
				    insn.immediateSizeRead
					[insn.numImmediatesConsumed];
			}
